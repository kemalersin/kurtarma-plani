...
**Grafik:** ECharts dashboard + ayrı analiz/rapor sayfaları. Üst KPI **`KpStatRow`** (`HomeView`: `two-by-two-until-lg`; «Bu ay net» `mobileHintToggle` gelir/gider). Tüm grafikler **`KpChart.vue`** sarıcısından geçer — tree-shaken `echarts/core` + `CanvasRenderer` + `BarChart`/`LineChart`/`PieChart` + `Grid`/`Legend`/`Title`/`Tooltip`/`DataZoom` (yeni grafik tipi gerekirse `use([...])` listesine eklenir). Responsive `ResizeObserver`; tema değiştiğinde dispose + re-init (`useUiStore.isDark`); `isEmpty` durumu boş-mesaj overlay'iyle.
